...
void Triangulation::verticalEdgeTagging(const double dotAngle)
{
 Node *n;
 Edge *e;
 Point up(0, 0, 1);
 ...
 FOREACHEDGE(e, n){
  if (e->t1 != NULL && e->t2 != NULL){
    Point n1 =  e->t1->getNormal();
    Point n2 =  e->t2->getNormal();
...
void Triangulation::shortEdgeTagging(const double d)
{
 Node *n;
 Edge *e;
 FOREACHEDGE(e, n)
  if (e->length() < d) TAG_SHARPEDGE(e);
  else UNTAG_SHARPEDGE(e);
}
