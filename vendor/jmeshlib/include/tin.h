...
void verticalEdgeTagging(const double dotAngle);
void shortEdgeTagging(const double d);
...
#define TIN_FOREACHVERTEX(Tt, n) for (n = tin.V.head(), Tt = (n)?((Vertex *)n->data):NULL; n != NULL; n=n->next(), Tt = (n)?((Vertex *)n->data):NULL)
#define TIN_FOREACHEDGE(Tt, n) for (n = tin.E.head(), Tt = (n)?((Edge *)n->data):NULL; n != NULL; n=n->next(), Tt = (n)?((Edge *)n->data):NULL)
